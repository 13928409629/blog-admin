          ...
          <a-row :gutter="48">
            ...
            <a-col :md="6" :sm="24">
              <a-form-item label="登录账号">
                <a-input v-model="queryParam.loginid" placeholder="登录账号" allowClear />
              </a-form-item>
            </a-col>
            <a-col :md="6" :sm="24">
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        ...
        {title: '登录账号',dataIndex: 'loginid',key: 'loginid',width: 300 },
  ...
  methods: {
    // 重置密码
    handleResetClick(r) {
      resetUserPassword({id: r._id}).then(res => {
        if(res.code == 200) {
    ...
    handleStatusChange(r) {
      updateUser({id: r._id,status: r.status?1:0,username: r.username}).then(res => {
        this.showMessage(res, this.getList)
      })
    }
  },
